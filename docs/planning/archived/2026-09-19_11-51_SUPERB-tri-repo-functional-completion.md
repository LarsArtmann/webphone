...
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- | ---------- | -------- | -------------- |
| ~~T01~~ | ~~pbx-artmann webphone config: `phoneApi.enable=true`, contacts, gateway (mode/url/settings via env), `environmentFile` wiring~~ done — executed 2026-09-19 (13:19 self-review = the record)                                       | ~~pbx-artmann~~ | ~~🔥🔥🔥~~ | ~~45m~~  | ~~—~~          |
| ~~T02~~ | ~~Messaging bridge: `telnyx-webhooks.py` rewrite (inbound→`/hooks/message`, status→`/hooks/message/status`, `/gateway/message`→Telnyx API, `/gateway/fax` 501, LoadCredential secrets) + unit test~~ done — executed 2026-09-19    | ~~pbx-artmann~~ | ~~🔥🔥🔥~~ | ~~100m~~ | ~~T01~~        |
| ~~T03~~ | ~~`generate.sh` secret list + `webhooks.nix` unit wiring (LoadCredential) + docs (AGENTS/CHANGELOG/TODO)~~ done — executed 2026-09-19                                                                                              | ~~pbx-artmann~~ | ~~🔥🔥~~   | ~~40m~~  | ~~T02~~        |
| ~~T04~~ | ~~webphone module hardening: proxySettings, `/events` location, `memoryMax`, dataDir assertion~~ done — executed 2026-09-19                                                                                                        | ~~webphone~~    | ~~🔥🔥~~   | ~~60m~~  | ~~—~~          |
| ~~T05~~ | ~~webphone flake QoL: devShell env attrs, `lib.*`, drop `with pkgs`, alias, fileset src, module-check vhost assertions~~ done — executed 2026-09-19                                                                                | ~~webphone~~    | ~~🔥~~     | ~~50m~~  | ~~T04~~        |
| ~~T06~~ | ~~webphone gates: `BUILDFLOW_NO_RESULT_CACHE=1 buildflow`, `GOEXPERIMENT=jsonv2 go test -count=1 ./...`, `nix flake check`, aarch64 pkg, smoke~~ done — executed 2026-09-19                                                        | ~~webphone~~    | ~~gate~~   | ~~45m~~  | ~~T04,T05~~    |
| ~~T07~~ | ~~webphone docs: AGENTS integration facts, TODO harvest, CHANGELOG Unreleased; commit + push~~ done — executed 2026-09-19                                                                                                          | ~~webphone~~    | ~~🔥~~     | ~~30m~~  | ~~T06~~        |
| ~~T08~~ | ~~Stack render fixes (contactsJson, phoneApi flag) + config.js contract assertion in `tests/webphone.nix` (if missing)~~ done — executed 2026-09-19                                                                                | ~~stack~~       | ~~🔥🔥~~   | ~~60m~~  | ~~—~~          |
| ~~T09~~ | ~~Re-pin webphone input in stack (verify push landed via `git ls-remote`); stack gates: `nix flake check` + webphone VM test check~~ done — executed 2026-09-19                                                                    | ~~stack~~       | ~~gate~~   | ~~40m~~  | ~~T07 (push)~~ |
| ~~T10~~ | ~~Browser E2E `nix build -L .#telephony-browser` against the new pin; stack docs (CHANGELOG/TODO)~~ done — executed 2026-09-19 (E2E green)                                                                                         | ~~stack~~       | ~~gate~~   | ~~60m~~  | ~~T09~~        |
| ~~T11~~ | ~~pbx-artmann local gates: toplevel build (x86) + `nix flake check` subset; ensure stack tree clean for re-lock~~ done — executed 2026-09-19                                                                                       | ~~pbx-artmann~~ | ~~gate~~   | ~~30m~~  | ~~T03,T10~~    |
| ~~T12~~ | ~~**Deploy**: SSH-create secrets (`openssl rand`) → `nix flake update` → `nixos-rebuild switch --flake .#pbx --target-host root@pbx.artmann.tech`~~ done — owner deploy; premise corrected 2026-09-22 (prod verified v2.4.0)       | ~~pbx-artmann~~ | ~~🔥🔥🔥~~ | ~~45m~~  | ~~T11~~        |
| ~~T13~~ | ~~**Verify**: probe suite (`/healthz`, `/config.js` phoneApi+contacts, `/` 200, hooks 401/503), service states, loopback `/gateway` self-test, one marked inbound test message~~ done — probes via smoke --base (16/0, 2026-09-22) | ~~prod~~        | ~~🔥🔥🔥~~ | ~~30m~~  | ~~T12~~        |
| ~~T14~~ | ~~Final docs pass + session summary + report~~ done — (docs synced; residue routed to the stack repo + owner rows)                                                                                                                 | ~~all~~         | ~~🔥~~     | ~~20m~~  | ~~T13~~        |
