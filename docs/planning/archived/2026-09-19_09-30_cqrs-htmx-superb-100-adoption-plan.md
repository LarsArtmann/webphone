...
3. The CQRS dispatch layer, `setup`, `usermgmt`, `loginpage`, casbin, and `RecommendedSecurityMiddleware` stay **rejected** (split-brain identity / microphone denial). "Superb" means superb _within_ the root-library posture.
...
`httputil.CSRFTokenHXHeaders(r)` HTML-escapes its JSON (`&quot;`) for **raw-HTML/single-quoted-attribute** contexts. templ escapes attribute values _itself_, so feeding the helper through a templ attribute would double-escape: the browser decodes once, `hx-headers` receives literal `&quot;…&quot;`, `JSON.parse` fails, and **every HTMX request loses its CSRF header** — a real Verschlimmbesserung.

The superb adoption in a templ codebase is therefore:

- `bodyAttrs` builds the header value with `templ.JSONString(map[string]string{"X-CSRF-Token": token})` — JSON-valid by construction (survives a quote inside a token) _and_ escaped exactly once by templ.
...
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ------- | ----------------------------------------------------------------------- | ------- | --------------------------------------------------- |
| ~~T2~~ | ~~Request-ID/correlation enrichment: `ContextEnrichmentMiddleware` + extension-as-`user_id` extractor; assert `request_id` in logs + `X-Request-ID` header~~ done at `d815004` | ~~High~~ | ~~Low~~ | ~~3 a.m. traceability of every request/response pair~~ | ~~40 min~~ | ~~new middleware test~~ |
| ~~T1~~ | ~~CSRF wiring hardening: `templ.JSONString` for `hx-headers`; `InvalidateCSRFCookie` on `POST /api/session` + `DELETE /api/session`~~ done at `d815004` | ~~High~~ | ~~Low~~ | ~~CSRF fixation closed; token-shape-proof JSON~~ | ~~45 min~~ | ~~new session/csrf tests + `templ generate`~~ |
| ~~T4~~ | ~~`Permissions-Policy: microphone=(self), camera=(), display-capture=(), geolocation=(), payment=(), usb=()` + extract single-source `securityHeadersConfig()` (kills the server.go ↔ middleware_test.go literal duplication)~~ done — Permissions-Policy shipped v2.1.0 | ~~Medium~~ | ~~Low~~ | ~~Calibrated hardening; mic still works; config drift structurally dead~~ | ~~30 min~~ | ~~header assertion test~~ |
| ~~T3~~ | ~~Replace hand-rolled `timingWriter`+`timingMiddleware` (~40 lines) with `servertiming.ServerTimingMiddlewareWhen` (auto `total;dur`, CRLF-sanitized, SSE-safe writer)~~ done — Server-Timing via httputil middleware (v2.1.0) | ~~Medium~~ | ~~Low~~ | ~~Same header, less code, named spans become possible~~ | ~~35 min~~ | ~~Server-Timing header test~~ |
| ~~T5~~ | ~~Webhook 5xx bodies: stop leaking `err.Error()` to providers — respond via `cqrshtmx.SafeDetail(err, 500, false)`, `slog.Warn` the detail server-side~~ done — webhook 5xx redaction shipped v2.1.0 | ~~Medium~~ | ~~Low~~ | ~~No internal detail on the wire; operators still see everything in logs~~ | ~~40 min~~ | ~~redaction test~~ |
| ~~T7~~ | ~~Verification & docs: full suite, BuildFlow gate, AGENTS.md correction (templ double-escape), CHANGELOG `[Unreleased]` entry, commits + push~~ done — gates + docs landed (landed list below) | ~~High~~ | ~~Medium~~ | ~~Durable knowledge; release-clean tree~~ | ~~50 min~~ | ~~BuildFlow green~~ |
| ~~T6~~ | ~~idiomorph experiment: serve `HTMXExtIdiomorph`, morph-swap the transcript panel, re-run **stack** browser E2E~~ done — idiomorph shipped v2.4.0 (cqrs-htmx v4.11.0) | ~~Medium~~ | ~~High~~ | ~~Draft-preserving SSE updates (kills the draft-wipe class)~~ | ~~60 min~~ | ~~**GATED: needs stack E2E — deferred, not skipped**~~ |
...
| Chain order | Enrichment _inside_ the request log                       | Enrichment **outermost**, log second                                                                      | `RequestLoggingSlog` reads the context after the handler returns — outside enrichment it can never see the RequestID (first test run proved it). Enrichment writes nothing, so the log's "sees every status" invariant is preserved.                                                         |
