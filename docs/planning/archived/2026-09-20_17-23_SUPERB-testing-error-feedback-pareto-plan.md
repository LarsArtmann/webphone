...
| ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~T11Session-persistence design spike (decision gate D2)~~             | ~~Turns the whole "dead session" failure class — silent 401s, toast throttles, restart UX — into a non-issue. Every other feedback task is a bandage on this wound.~~ |
| ~~T12Session-persistence implementation (gated on D2 approval + T11)~~ | ~~Ships the fix: sessions survive restarts, cookie unchanged, owner scoping unchanged, fail-closed posture unchanged.~~                                               |
...
| -------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~T01Toast accessibility (aria-live, role, keyboard dismissal)~~                 | ~~The entire feedback channel is currently INVISIBLE to screen-reader users. One attribute closes it.~~                                                          |
| ~~T02Error-body swap via htmx responseHandling (+ 502 toast pairing assertion)~~ | ~~The server already renders correct panel errors — htmx discards the bodies today. Wiring responseHandling turns transient toasts into durable inline errors.~~ |
| ~~T03Toast feedback-map completion (login-fail, 429, SSE-drop toasts)~~          | ~~Three error paths still end in `#log`-only or pill-only feedback.~~                                                                                            |
