...
> [2026-09-19 status report](../status/2026-09-19_00-05_cqrs-htmx-adoption-execution-status.md).
...
2. ~~`/healthz` probe semantics (hard gate vs informational) → gates H3 details/RC1~~ resolved by implementation — `cqrshtmx.ReadinessHandler` with named `sqlite`/`blob-dir` checks, GET-open by decision (`bbd74a1`, `2f6ffee`)
3. ~~`/events` byte-stream E2E freeze status → gates S1~~ resolved by verdict — connected frame is listener-inert, additive-only; upstream E2E skip documented (status 2026-09-19 §g.3)
...
- ~~Phase 3 exit: adoption score ≈ 80 (middleware adopted, ratelimit.go deleted, honest
  ...
  AGENTS.md/CHANGELOG, items harvested).~~ **Met** — exceeded: score 92 (`062445c`).
- ~~Phase 4 exit: score re-verified ≈ 80–90 with reproducible rubric; audit claims 100%
  tag-verified; nothing unread left in the server layer.~~ **Met** — 92 with rubric appendix, 100% tag-verified, server layer fully read (`062445c`).
- ~~Phase 7 exit (full 100%): every decision recorded, enhancements either adopted behind
  flags or parked with written verdicts, hardening tests in place.~~ **Met** — decisions in the rubric appendix/ROADMAP, OOB parked with written criteria, hardening tests shipped (`062445c`, `82efa2c`, status 2026-09-19 §a).
