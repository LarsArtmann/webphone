...
1. ~~Run the consuming stack's browser E2E against current `main` (this session changed headers/attributes).~~ done (stack browser E2E green across every train since; ×2 on the v2.5.0 chain 2026-09-22)
2. ~~T6: serve `cqrshtmx.HTMXExtIdiomorph` and evaluate `sse-swap="morph"` on the transcript panel.~~ done (shipped v2.4.0 — idiomorph morph-swap via cqrs-htmx v4.11.0)
3. ~~Island-side CSRF token refresh after login; then adopt `InvalidateCSRFCookie` server-side.~~ done at `d815004`
4. ~~Re-run the stack E2E after 2+3; flip the DOM-contract docs.~~ done (done (E2E re-runs + DOM-contract docs single-sourced in docs/dom-contract.md))
5. ~~Re-audit the adoption score post-T6 (expect ~100) and annotate the plan doc's score table.~~ done (superseded: adoption deepened far past the score (fuzz/VM/backups/morph/CRM since))

**Living docs / knowledge:**
6. ~~HARVEST this report's (f) into `TODO_LIST.md` (docs-health skill).~~ done (done (docs-health sweeps 2026-09-19 + 2026-09-22))
7. ~~Add a correction appendix pointer to the audit snapshot policy question (annotate vs. never-edit).~~ **Won't implement — snapshot stays immutable (annotate-never-rewrite policy).**
8. ~~`docs-health` VERIFY pass over AGENTS.md claims changed this session.~~ done (done (docs-health VERIFY passes; living docs current 2026-09-22))
9. ~~Annotate the 2026-09-19 07:43 status report with the landed state (it predates the adoption work).~~ done (done (07-43 report annotated by the 08:33 sweep))
10. ~~Record the BuildFlow daemon-race lesson (run gates on a quiet tree) in AGENTS.md.~~ done (runbook carries quiet-tree gate discipline + narrative-commit rule)

**Upstream watch:**
11. ~~Watch for cqrs-htmx v4.10.0 release; re-verify SSE behavior at the consumed tag before bumping.~~ done (v4.11.0 adopted (v2.4.0); v4.12.0 released upstream with objects)
12. ~~When go-sse ships the reconnect `retry:` hint downstream, re-check `ServeSSE` and update the AGENTS.md tag-checking note.~~ done (shipped: cqrs-htmx v4.11.0 SSE retry hint, stream-test-pinned)
13. ~~templ-components `ThemeScript` opt-out watch — if it ships, drop the CSP hash + `!important` color-scheme rules.~~ done (CLOSED 2026-09-22 — NoThemeScript shipped (v1.19.2) and consumed; CSP hash + !important dropped)
14. ~~httputil v1.3.0 watch (we track v1.2.0+1).~~ done (httputil servertiming middleware adopted v2.1.0)

**Product polish surfaced by the audit:**
15. ~~Atomic `claim` op for the webhook idem store (close the `seen`+`record` TOCTOU race).~~ done (superseded: hooksIdem dedup shipped (202-inert replays))
16. ~~Consider 503 (retryable) classification for transient webhook store failures — behind a provider-behavior check.~~ done (superseded: error families classify transport vs rejection (v2.5.0))
17. ~~`MeasureServerTiming` named spans around PBX phone-api calls (now free with the library middleware).~~ done (Server-Timing shipped v2.1.0 (library middleware); spans stay opt-in)
18. ~~Surface `X-Request-ID` in the island's `#log` lines so browser-side and server-side ids correlate.~~ done (X-Request-ID shipped v2.1.0; #log correlation stays island-side)
19. ~~Sweep remaining `err.Error()` interpolations in user-facing 4xx/422 panel errors (actions.go) for i18n parity.~~ done (superseded: sendFailure ladder + typed provider-rejection detail (v2.5.0))
20. ~~Audit the remaining `http.Error` sites for SafeDetail-style redaction outside webhooks (proxy.go 502s).~~ done (webhook 5xx redaction shipped v2.1.0; proxy stays redacted)
21. ~~OpenAPI spec: decide whether `/hooks/*` contracts deserve publication (currently only `/api/session`).~~ done (OpenAPI grew to /api/session + /api/contacts + /api/calls; /hooks stay doc-only (README contracts))
22. ~~`Notify*` vs `ToastDetail` shape trap: add a comment/alias guard in case the dispatch layer ever lands.~~ done (toast wire shape is a type alias of cqrshtmx.ToastDetail — upstream change fails the build)
23. ~~`OOBHTML` evaluation for the nav unread badges (avoid full-panel SSE pushes for badge-only updates).~~ done (PARKED by verdict docs/reviews/2026-09-18_oob-badge-spike-verdict.md)
24. ~~Settings panel: show the server version (`/version` payload) so operators see what runs.~~ done (/version shipped; settings panel keeps the honest disabled-state design)
25. ~~`SafeRedirectPath` adoption readiness review (only relevant if a server-side login redirect ever appears).~~ **Won't implement — no server-side login redirect exists; SafeRedirectPath not adopted.**

**Repo hygiene noticed this session:**
26. ~~`.buildflow.yml` `skip_steps` cleanup — `branching-flow`, `cqrs-lint`, `go-structure-linter`, `nix-hash-fix`, `pytest-test` "match no registered tool" (warning at every load).~~ done (documented-skip pattern in .buildflow.yml)
27. ~~Investigate the 9 BuildFlow tools "unavailable (health check failed)" (incl. `go-licenses` binary missing).~~ done (nine-tools warning documented as noise in AGENTS (go-licenses now in devShell))
28. ~~Review what the daemon's 3-line `flake.nix` change (commit `6691ca1`) actually was — I never inspected it.~~ done (flake.nix daemon churn long since replaced by real pins)
29. ~~Decide: explicit per-task commits vs. daemon-owned history (they raced each other all session).~~ done (narrative-commit-at-phase-boundary rule in AGENTS + runbook)
30. ~~Run gitleaks + codespell once (on-demand BuildFlow steps, skipped in every mode).~~ done (gitleaks + codespell default in scripts/buildflow.sh)
31. ~~Run `markdown-lint`/`lychee` over the three new docs (skipped in full mode).~~ done (markdown-lint/codespell run in buildflow full mode)
32. ~~`git log -S timingWriter` sanity: confirm no stale references in docs after the deletion.~~ done (timingWriter deletion verified clean long since)

**Testing depth:**
33. ~~Table-driven test for the enrichment middleware order (enrichment-inside-log regression would currently only fail via the wiring test — make the invariant explicit).~~ done (middleware order pinned by contract tests (enrichment-outside-log scan))
34. ~~Fuzz or property test for `hx-headers` rendering with adversarial tokens (quotes, unicode).~~ done (templ.JSONString regression test parses the rendered hx-headers (v2.1.0))
35. ~~Contract test: `Notify*` wire shape (`{level,message}`) vs island listener — pin the difference so a future dispatch-layer adoption fails loudly.~~ done (toast wire shape pinned by the type-alias contract)
36. ~~Race test for `ExtensionHubs` under concurrent publish+sweep (existing code, untested this session).~~ done (hub races covered: hub_bench + reaper tests; BenchmarkHubFanOut baseline doc)
37. ~~Smoke-test the built binary through `flake.nix` output (the AGENTS.md smoke command was never run this session).~~ done (smoke suite is the standing binary-level gate (38 checks))

**Release / ops:**
38. ~~Cut v2.1.0 from `[Unreleased]` (five library adoptions + redaction are user-visible).~~ done at `d815004`
39. ~~After release: `vulnix $(nix-store -qR ./result)` runtime-closure re-verification.~~ done (vulnix rides release.sh every train)
40. ~~Stack-side input bump to the post-adoption commit; record the switchover in AGENTS.md.~~ done (stack rides webphone main (DECIDED 2026-09-20))

**Bigger swings (ROADMAP fuel):**
41. ~~Event-catalog-driven SSE event naming audit (`threads`/`thread`/`fax`/`voicemail` vs a documented registry).~~ **Won't implement — event-naming registry not adopted; events documented in AGENTS.**
42. ~~Session-store persistence decision doc (in-memory by design — write down the multi-instance caveat).~~ done (superseded: SQLite session store shipped (2.5.0))
43. ~~`RenderTemplComponent`-style local helper to DRY the repeated Content-Type+render+error pattern (7 sites).~~ **Won't implement — render helper not extracted; templ render sites stay explicit.**
44. ~~Evaluate `HTMXMiddleware` + HX-* context accessors if any handler ever needs request-aware partials.~~ **Won't implement — HTMXMiddleware not adopted.**
45. ~~Journal-backed SSE replay (`JournalSSEStore`) if message history needs reconnect-replay.~~ **Won't implement — SSE replay not adopted (morph + refetch covers live surfaces).**
46. ~~`datastar` evaluation spike (same hub, different transport) — only if HTMX limits appear.~~ **Won't implement — datastar spike not adopted.**
47. ~~`adminui`/`dashboardui` adoption spike if operator surfaces are ever wanted.~~ **Won't implement — adminui/dashboardui not adopted.**
48. ~~`IdempotencyStore` upstream API check: propose a record-on-success variant (webhook semantics) upstream.~~ **Won't implement — IdempotencyStore upstream proposal not filed; local idem store converged.**
49. ~~Performance baseline: Server-Timing spans + `benchmark_*` comparison before/after chain changes.~~ **Won't implement — baseline spans not adopted beyond opt-in Server-Timing.**
50. ~~Docs: a one-page "which library owns which surface" map (cqrs-htmx / httputil / go-sse / templ-components) from the audit tables.~~ **Won't implement — library-map doc not written; AGENTS carries the consumed-surface notes.**
