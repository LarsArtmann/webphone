...
- **What did I forget?** (1) Push-state verification (`git ls-remote`) until report time — AGENTS.md makes it a standing rule. (2) `-race` over the idempotency-adjacent server refactor (prior session's precedent). (3) Dedicated unit tests for every new helper — they are pinned only _via callers_ (the avatarFor lesson says that is how first cuts ship bugs). (4) Root-causing the art-dupl version drift instead of asserting it. (5) Naming, in "green" claims, exactly which gates did NOT run.
