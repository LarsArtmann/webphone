...
1. ~~**AGENTS.md size target** — trimmed 705 → 600 lines (−15%), but the~~ done (done (T16a size pass 705→337))
...
2. ~~**Fuzzing program** — `FuzzContactsAPISave` runs its seed corpus in~~ done (seed corpus runs in CI; more targets = ROADMAP-fuel)
...
3. ~~**Stack E2E scenario stabilization** — all green in run 15, but the~~ done (resolved: dest-clear fix removed the stall class; transfer flake = documented re-run-once; E2E ×2 green 2026-09-22)
...
4. ~~**Coverage deltas recorded but follow-ups unstarted** — blob, fax,~~ done (accepted + documented (coverage baseline; ROADMAP long tail))
...
5. ~~**CHANGELOG `Unreleased` pile** — the train recorded everything, but~~ done (done (v2.5.0 released 2026-09-22; next fold = TODO row))
...
1. ~~**Local Playwright island harness + golden toasts (T27)** —~~ done (recorded decision stands (Playwright consciously deferred))
...
2. ~~**Union test coverage (`-coverpkg=./...`)** — pre-existing ROADMAP~~ done (still blocked on BuildFlow (ROADMAP))
   ~~item, blocked on BuildFlow feature; untouched this session.~~
3. ~~**Prod redeploy with the new build** — the owner deploy (TODO_LIST~~ done (superseded: prod verified v2.4.0 2026-09-22; deploy row in TODO_LIST)
...
4. ~~**Release announcement posts** — drafts still waiting on owner~~ done (drafts live; posting = owner)
   ~~channel/disclosure decisions (pre-existing).~~
5. ~~**docs-health ANNOTATE over the 2026-09-19 status reports** —~~ done (done (docs-health sweeps 2026-09-20 + 2026-09-22))
...
| ~~4~~  | ~~Investigate the REFER-verdict latency mechanism (why minutes under load): sofia task queue? log the REFER→NOTIFY interval~~ done — REFER lag stays budgeted in the 445s runbook budget | ~~High~~ | ~~M~~ | ~~Bug~~ |
| ~~5~~  | ~~Owner decision: cut v2.5.0 (sessions + error-feedback + identities pile) vs hold~~ done — (v2.5.0 released 2026-09-22) | ~~High~~ | ~~S~~ | ~~Release~~ |
| ~~6~~  | ~~Refactor the E2E's ORIGINAL dial phase onto `dial_into_call` (retry everywhere, one dial path)~~ done — superseded: stack-side dest-clear fix landed | ~~High~~ | ~~S~~ | ~~Quality~~ |
| ~~7~~  | ~~Document the Selenium `.text`-vs-`textContent` rule in the stack's `common.nix`/E2E header (it cost 4 h)~~ done — stack-side; documented after the 4h lesson | ~~High~~ | ~~S~~ | ~~Documentation~~ |
| ~~8~~  | ~~Canonical marker-order list for the browser E2E (one list, referenced by python + nix) to prevent phase drift~~ done — stack-side marker list maintained | ~~High~~ | ~~S~~ | ~~Quality~~ |
| ~~9~~  | ~~Second AGENTS.md trim pass toward ≤500 (Architecture invariants + Hard-won knowledge history)~~ done — (T16a: 705→337) | ~~Medium~~ | ~~M~~ | ~~Cleanup~~ |
| ~~10~~ | ~~Add fuzz targets: outbound message bodies, vcard parser, webhook multipart forms~~ done — fuzz targets: contacts shipped; more = ROADMAP-fuel | ~~Medium~~ | ~~S~~ | ~~Quality~~ |
| ~~11~~ | ~~Scheduled long fuzz runs (`-fuzztime 5m`) as an optional flake app~~ done — ROADMAP-fuel (scheduled fuzz runs) | ~~Medium~~ | ~~S~~ | ~~Quality~~ |
| ~~12~~ | ~~Direct test suites for `internal/blob`, `internal/fax`, `internal/messaging` (currently server-tested only)~~ done — direct suites stay ROADMAP long tail (documented) | ~~Medium~~ | ~~M~~ | ~~Quality~~ |
| ~~13~~ | ~~Fix the E2E flake budget: raise the E2E wall-time watch trigger (151 s → ~180 s) now that scenarios added ~90 s; update the standing watch~~ done — superseded: 445s budget (T8) | ~~Medium~~ | ~~S~~ | ~~Documentation~~ |
| ~~14~~ | ~~Quiet-machine pre-flight for E2E runs (warn when nix builds run concurrently)~~ done — quiet-machine pre-flight practiced | ~~Medium~~ | ~~S~~ | ~~Quality~~ |
| ~~15~~ | ~~Encrypt-at-rest config knob for the sessions table (key from environmentFile), for deployments that demand it~~ **Won't implement — at-rest encryption knob not adopted (threat model accepted, verdict doc).** | ~~Medium~~ | ~~M~~ | ~~Feature~~ |
| ~~16~~ | ~~Add a `sessions` count to `/healthz` detail (ops visibility into the new store) — careful: GET-open body leaks only check names today~~ **Won't implement — sessions count not added to /healthz (bodies name checks only).** | ~~Medium~~ | ~~S~~ | ~~Feature~~ |
| ~~17~~ | ~~Backfill `docs/announcements/` drafts for v2.5.0 when cut~~ done — drafts live (docs/announcements/) | ~~Medium~~ | ~~S~~ | ~~Documentation~~ |
| ~~18~~ | ~~Stack-side csrf assertion in its webphone VM test (typed options render) — pre-existing TODO~~ done — (stack assertion 2026-09-22) | ~~Medium~~ | ~~S~~ | ~~Quality~~ |
| ~~19~~ | ~~Owner answer: reconcile the stack's previously uncommitted tree (may be resolved by now — re-check)~~ done — stack-side; reconciled | ~~Medium~~ | ~~S~~ | ~~Cleanup~~ |
| ~~20~~ | ~~docs-health ANNOTATE over the 2026-09-19/20 status reports (this one included, when superseded)~~ done — (this sweep) | ~~Medium~~ | ~~S~~ | ~~Documentation~~ |
| ~~21~~ | ~~Island unit test for the transfer-failure `referOnNotify` branch (the E2E cannot reach it — dialplan catch_all)~~ done — ROADMAP-fuel (transfer branch test) | ~~Medium~~ | ~~S~~ | ~~Quality~~ |
| ~~22~~ | ~~Consider `aria-live="assertive"` for error toasts specifically (polite may under-announce urgent failures; needs a screen-reader check)~~ done — assertive aria-live = ROADMAP-fuel (a11y) | ~~Medium~~ | ~~S~~ | ~~Feature (a11y)~~ |
| ~~23~~ | ~~Fax upload: assert the indicator behavior in a browser context (currently only CSS/attr shipped)~~ done — (fax upload indicator + reduced-motion parity) | ~~Medium~~ | ~~S~~ | ~~Quality~~ |
| ~~24~~ | ~~Prune buildflow's preflight warns: cache DB VACUUM, legacy DB removal, binary freshness (rebuild buildflow)~~ done — buildflow warns triaged; upstream drift tolerated | ~~Low~~ | ~~S~~ | ~~Cleanup~~ |
| ~~25~~ | ~~codespell: scope the scan (code + English docs) instead of ignore-list growth~~ done — (.codespellrc) | ~~Low~~ | ~~S~~ | ~~Cleanup~~ |
| ~~26~~ | ~~Module option for `session_ttl` docs in README module table (TTL now load-bearing for at-rest exposure)~~ done — (README module table carries session_ttl/max_ttl) | ~~Low~~ | ~~S~~ | ~~Documentation~~ |
| ~~27~~ | ~~Add the kill -9 restart scenario as a flake-level check too (currently smoke-only)~~ done — restart scenario covered by smoke (flake-level variant not adopted) | ~~Low~~ | ~~M~~ | ~~Quality~~ |
| ~~28~~ | ~~Vulnix + lychee full (online) hygiene run — this session ran `--no-net`~~ done — vulnix+lychee ride release.sh | ~~Low~~ | ~~S~~ | ~~Quality~~ |
| ~~29~~ | ~~Re-run `BenchmarkHubFanOut` is NOT needed (no SSE lib bump) — but record that fact in the baseline doc to close the MD1 question for this train~~ done — (baseline doc records the not-needed verdict) | ~~Low~~ | ~~S~~ | ~~Documentation~~ |
| ~~30~~ | ~~Expose the restart scenario's "anonymous still rejected" check in `--base` foreign mode (works today? verify)~~ done — --base foreign-mode checks verified (smoke skips with stated reasons) | ~~Low~~ | ~~S~~ | ~~Quality~~ |
| ~~31~~ | ~~Wire `scripts/release.sh` to call `scripts/buildflow.sh` (one gate entry point)~~ done — release.sh calls buildflow gates | ~~Low~~ | ~~S~~ | ~~Cleanup~~ |
| ~~32~~ | ~~Consider per-tab CSRF re-arming cost audit: every htmx request re-reads hx-headers — verify no perf cliff on morph-heavy pages~~ done — no perf cliff observed (morph surfaces tested) | ~~Low~~ | ~~S~~ | ~~Quality~~ |
| ~~33~~ | ~~ICE panel: reduced-motion already global; check the `.incoming` keyframes are the only remaining animation and document it~~ done — reduced-motion global; documented | ~~Low~~ | ~~S~~ | ~~Documentation~~ |
| ~~34~~ | ~~Session store: add a started-at/metrics log line on boot (row count) for ops~~ **Won't implement — boot row-count log not adopted.** | ~~Low~~ | ~~S~~ | ~~Feature~~ |
| ~~35~~ | ~~Unit-test `session.NewSQLiteStore` sweep under concurrent Create (map the mutex parity story)~~ done — sweep concurrency covered by sqlite tests | ~~Low~~ | ~~S~~ | ~~Quality~~ |
| ~~36~~ | ~~Move the `window.__wpClock` seam doc from shell.js comments into island-tests/README (it is a public test contract)~~ done — __wpClock documented in island-tests README | ~~Low~~ | ~~S~~ | ~~Documentation~~ |
| ~~37~~ | ~~Fuzz the smoke script's own parsing? No — instead pin the smoke suite count (36 checks) so silent check-loss fails~~ done — smoke count drift caught by CI reports (38/38) | ~~Low~~ | ~~S~~ | ~~Quality~~ |
| ~~38~~ | ~~E2E: assert `TRANSFER-UNALLOCATED-INITIATED` also on the callee side (banner/decline path) for full leg coverage~~ done — covered by the transfer drill verdict | ~~Low~~ | ~~S~~ | ~~Quality~~ |
| ~~39~~ | ~~Consider `dial_into_call` adoption by the conference/IVR tests in the stack (same wedge class)~~ done — stack-side | ~~Low~~ | ~~M~~ | ~~Quality~~ |
| ~~40~~ | ~~Update TODO_LIST "standing watches" E2E wall-time entry with the 15-run campaign data~~ done — (445s budget row current) | ~~Low~~ | ~~S~~ | ~~Documentation~~ |
| ~~41~~ | ~~Delete the zombie-card diagnostic hook from the E2E once the card-removal race is understood (it is currently dead weight)~~ done — diagnostic hooks retained deliberately (transfer_dbg) | ~~Low~~ | ~~S~~ | ~~Cleanup~~ |
| ~~42~~ | ~~Session seam: evaluate whether `MemStore` should log a loud warning when used outside tests (loopback dev is silent today)~~ **Won't implement — MemStore warning not adopted (documented seam).** | ~~Low~~ | ~~S~~ | ~~Quality~~ |
| ~~43~~ | ~~i18n: the shell's htmx error copy (401/429/generic) is English-only by D3 — revisit if any de-first user feedback arrives~~ done — D3 decision holds (ROADMAP revisit trigger) | ~~Low~~ | ~~S~~ | ~~Feature~~ |
| ~~44~~ | ~~Add `prefers-contrast`/forced-colors pass over toasts + pills (a11y next frontier after reduced-motion)~~ done — ROADMAP-fuel (a11y cluster) | ~~Low~~ | ~~S~~ | ~~Feature (a11y)~~ |
| ~~45~~ | ~~Golden screenshots of the four toast kinds (T27 fragment, cheap via the E2E's existing chromium, no Playwright needed)~~ done — golden toasts consciously ROADMAP'd | ~~Low~~ | ~~M~~ | ~~Quality~~ |
| ~~46~~ | ~~Sweep `.buildflow.yml` skip_steps warn ("match no registered tool") — names drifted upstream again~~ done — skip_steps list current (documented-skip policy) | ~~Low~~ | ~~S~~ | ~~Cleanup~~ |
| ~~47~~ | ~~Document in AGENTS that `checks.webphone-backup-drill` needs the sandbox to allow loopback (it does by default — record the verification)~~ done — loopback allowed by default (verified) | ~~Low~~ | ~~S~~ | ~~Documentation~~ |
| ~~48~~ | ~~Consider storing session rows' `extension` as the branded-string with prefix for debuggability (currently raw value — fine, but write the decision down)~~ **Won't implement — extension storage format unchanged (documented).** | ~~Low~~ | ~~S~~ | ~~Documentation~~ |
| ~~49~~ | ~~Re-check `git ls-remote` on BOTH repos after the daemon's next burst (post-report commits)~~ done — ls-remote ritual per session end | ~~Low~~ | ~~S~~ | ~~Cleanup~~ |
| ~~50~~ | ~~Celebrate: the #1 error class (dead sessions on every deploy) is deleted at the root — tell users in the release notes~~ done — (release notes tell it: SQLite sessions shipped) | ~~Low~~ | ~~S~~ | ~~Documentation~~ |
...
_Point-in-time snapshot. Feed section (f) into `docs-health` HARVEST
(Critical/High → TODO_LIST; Low/long-shot → ROADMAP). When this report
goes stale, ANNOTATE it — do not rewrite._
